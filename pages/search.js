...
export default function Search() {
  ...
  const NoQuery = () => {
    return (
      <div className="flex flex-col items-center justify-center text-lg text-gray-400 dark:text-gray-300 p-4 text-center py-20">
        ...
        <p>
          <br />
          If we&apos;ve missed an item, please send us some{" "}
          <Link href="/library?feedback=open">
            <a className="font-bold underline">feedback</a>
          </Link>{" "}
  ...
  const Results = connectStateResults(({ searchState, children }) =>
    searchState && searchState.query ? <>{children}</> : <NoQuery />
  );
      ...
      <div className="relative">
        ...
        {currentRefinement ? (
          <MdCancel
            ...
            size="1.25em"
            className="cursor-pointer text-gray-300 absolute top-[9px] right-3"
          />
        ) : null}
        <MdOutlineSearch
          size="1.25em"
          className="text-gray-500 top-3 left-3 absolute"
        />
      </div>
    );
  };
