    ...
    <div className="p-4 w-full h-full flex flex-col items-center max-w-screen-2xl">
      <div className="flex flex-col items-center lg:items-end lg:self-start lg:flex-row lg:">
        <div className="p-10 pb-2 relative active:scale-95 transition-all lg:p-0 lg:pt-24 lg:max-w-[260px] lg:mr-6">
          <Image
            priority
            alt={`${listDoc.title} album cover`}
            height={380}
            width={380}
            src={coverArt}
            className="rounded-xl aspect-square object-cover"
          />
          <div className="absolute right-5 top-16 lg:top-28 lg:-right-5">{listDoc.featured ? <FeaturedPill message="Featured" /> : null}</div>
        </div>
        <div className="flex flex-col items-center lg:items-start">
          <h1 className="font-bold text-lg">{listDoc.title}</h1>
          <h2 className="text-xs mb-5">{isPlaylist ? `Playlist featuring ${Array.from(artists).join(', ')}` : `${listDoc.artist} - ${listDate && listDate}`}
          </h2>
        </div>
      </div>
